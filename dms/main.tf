...
data "template_file" "rep-settings" {
  template = "${file("${path.module}/replication_task_settings.json")}"
}
data "template_file" "tab-settings" {
...
resource "aws_dms_replication_task" "task" {
  ...
  replication_task_settings = "${data.template_file.rep-settings.rendered}"
  table_mappings            = "${data.template_file.tab-settings.rendered}"
